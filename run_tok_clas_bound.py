...
def main():
    ...
    parser.add_argument("--noise_probability", type=float, default=0.5,
                        help="Probability to mask out each boundary.")
    ...
    def mask_boundary(input_ids, label_ids, noise_probability=0.5):
        device = input_ids.device
        input_ids, label_ids = input_ids.cpu().clone().numpy(), label_ids.cpu().clone().numpy()
        probability_matrix = []
        ...
        return torch.from_numpy(input_ids).to(device), torch.from_numpy(label_ids).to(device)
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                input_ids, label_ids = mask_boundary(input_ids, label_ids, args.noise_probability)
